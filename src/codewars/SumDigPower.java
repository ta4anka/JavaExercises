...
(inclusive) and outputs a list of the sorted numbers
...
public class SumDigPower {
    public static List<Long> sumDigPow(long a, long b) {
        ...
        for(long i = a; i<= b; i++) {
            long CountDig = Long.toString(i).length();

            long SumOfPowers = 0;

            for(long j = i; j > 0;j /= 10) {
                SumOfPowers += Math.pow((j % 10), CountDig--);
                }
                if(i == SumOfPowers ){
                aList.add(i);
            }
    ...
    public static void main(String[] args) {
        System.out.println(sumDigPow(1, 1000)); //[1, 2, 3, 4, 5, 6, 7, 8, 9, 89, 135, 175, 518, 598]
    }
}
